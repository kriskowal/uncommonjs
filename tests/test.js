...
exports['test ok false'] = function () {
    assert.error(makeBlock(assert.ok, false), assert.AssertionError);
};
...
exports['test equal true false'] = function () {
    assert.error(makeBlock(assert.equal, true, false), assert.AssertionError, 'equal');
};
...
exports['test notEqual true true'] = function () {
    assert.error(makeBlock(assert.notEqual, true, true), assert.AssertionError, 'notEqual');
};
exports['test strictEqual 2 "2"'] = function () {
    assert.error(makeBlock(assert.strictEqual, 2, "2"), assert.AssertionError, 'strictEqual');
};
exports['test strictEqual null undefined'] = function () {
    assert.error(makeBlock(assert.strictEqual, null, undefined), assert.AssertionError, 'strictEqual');
};
exports['test notStrictEqual 2 "2"'] = makeBlock(assert.notStrictEqual, 2, "2");

//deepEquals

exports['test deepEqual date'] = makeBlock(assert.deepEqual, new Date(2000,3,14), new Date(2000,3,14));
exports['test deepEqual date negative'] = function () {
    assert.error(makeBlock(assert.deepEqual, new Date(), new Date(2000,3,14)), assert.AssertionError, 'deepEqual date');
};

exports['test deepEqual 4 "4"'] = makeBlock(assert.deepEqual, 4, "4");
exports['test deepEqual "4" 4'] = makeBlock(assert.deepEqual, "4", 4);
exports['test deepEqual true 1'] = makeBlock(assert.deepEqual, true, 1);
exports['test deepEqual 4 "5"'] = function () {
    assert.error(makeBlock(assert.deepEqual, 4, "5"));
};

// having the same number of owned properties && the same set of keys
exports['test deepEqual {a:4} {a:4}'] = makeBlock(assert.deepEqual, {a:4}, {a:4});
exports['test deepEqual {a:4,b:"2"} {a:4,b:"2"}'] = makeBlock(assert.deepEqual, {a:4,b:"2"}, {a:4,b:"2"});
exports['test deepEqual [4] ["4"]'] = makeBlock(assert.deepEqual, [4], ["4"]);
exports['test deepEqual {a:4} {a:4,b:true}'] = function () {
    assert.error(makeBlock(assert.deepEqual, {a:4}, {a:4,b:true}), assert.AssertionError);
};
...
exports['test deepEqual arrays with non-numeric properties'] = function () {
    ...
    a2.b = true;
    a2.a = "test"
    assert.error(makeBlock(assert.deepEqual, Object.keys(a1), Object.keys(a2)), assert.AssertionError);
...
exports['test deepEqual identical prototype'] = function () {
    ...
    assert.error(makeBlock(assert.deepEqual, nb1, nb2), assert.AssertionError);

};

exports['test deepEqual "a" {}'] = function () {
    assert.error(makeBlock(assert.deepEqual, 'a', {}), assert.AssertionError);
};
...
exports['test deepEqual "" [""]'] = function () {
    assert.error(makeBlock(assert.deepEqual, '', ['']), assert.AssertionError);
};
...
exports['test throw AssertionError'] = function () {
    ...
    assert.error(makeBlock(thrower, assert.AssertionError), assert.AssertionError, 'message');
    assert.error(makeBlock(thrower, assert.AssertionError), assert.AssertionError);
    assert.error(makeBlock(thrower, assert.AssertionError));
    //if not passing an error, catch all.
    assert.error(makeBlock(thrower, TypeError));
    //when passing a type, only catch errors of the appropriate type
    var threw = false;
    try {
        assert.error(makeBlock(thrower, TypeError), assert.AssertionError);
    } catch (e) {
        threw = true;
